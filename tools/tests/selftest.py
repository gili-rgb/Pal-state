...
expect("marom_bad", FIX / "marom_bad.html", "marom", must_have_errors=[
    ...
    "SCHEMA_PRODUCT_BLOG", "NAP_ADDR", "NAP_PHONE", "BRAND_LINK_UNKNOWN",
])

expect("marom_good", FIX / "marom_good.html", "marom", must_pass=True,
       expect_type="blog")

# v1.5.0 — כללי הריצה של "וו גיטרה" (מרום, 2026-08-03)
expect("connectors_bad", FIX / "connectors_bad.html", "marom", doc_type="blog",
       must_have_errors=["SOURCES_INTERNAL_TOOL"])

def expect_warn(name, path, site, rules, doc_type=None):
    rep, _, _ = run(path, site, doc_type)
    got = {w["rule"] for w in rep.warns}
    missing = set(rules) - got
    if missing:
        FAILED.append(f"{name}: אזהרות שלא נתפסו: {sorted(missing)} | נתפסו: {sorted(got)}")
    else:
        print(f"✅ {name}: כל {len(rules)} האזהרות נתפסו")
