# PAL STATE
עודכן: 2026-08-16 (v8.19, pal-lint v1.14.0, version_guard v1.3, 8 שערי CI, שישה מקורות מדידה)
...
|------|------|-------|------|
| content-machine | **v8.19** | מותקן. נהג של preflight/postflight | ריצת אימות ראשונה בוצעה; מאמר Refresh פורסם 2026-08-10 |
...
- **2026-08-17 (כפתור חיוג שבור עלה לאוויר):** במאמר הראשון שנכתב בארכיטקטורה החדשה, שלושת כפתורי החיוג היו `tel:0899777222` — **ספרה עודפת**, וכל לחיצה ממובייל נכשלה. **הבאג עבר את שמונת השערים.** תוקן ב-`TEL_LINK_INVALID` ו-`TEL_LINK_MISMATCH` (ERROR) שמאמתים כל `tel:` מול ה-NAP. בנוסף `AI_CHANNEL_ORDER` השווה מחרוזות קשיחות והיה עיוור לכתיב `08-9777222`; במאמר הטלפון הופיע בתו 6,896 והאזור האישי בתו 18,286, כלומר המוקד ראשון. **לקח: כלל שמשווה מחרוזת קשיחה שווה לכלל שאינו קיים ברגע שהכתיב משתנה.**
- **2026-08-17 (מסלול פעולה דיגיטלי):** `AI_CHANNEL_MISSING` כפה קישור לנציגת ה-AI על כל תוכן שירות, אבל **מאיה ודנה מטפלות בתיאום התקנת מוצר חדש בלבד**. לקוח עם מכשיר תקול נשלח לערוץ שגוי, נכשל, ומתקשר למוקד. **יעד-העל: לסיים את מסע הלקוח בדיגיטל ולהוריד כוח אדם במוקד; חיוג = עדיפות אחרונה.** שני מסלולים: התקנה ← נציגת AI (קול וצ'אט), כל השאר ← **אזור אישי**: CSB `myarea.csb.co.il`, מרום `myarea.marom-serv.co.il/login/`, פלרום `myarea.plrom.co.il/login/`. **לפלרום אין נציגה אבל כן אזור אישי — מסלול הפעולה הדיגיטלי היחיד שלה.** כלל `SELF_SERVICE_MISSING`, שדה `brief.self_service`.
