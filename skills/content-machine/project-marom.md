...
> מקור: מיפוי GSC חי (`cats/marom_page_queries.json`, 2026-08-16). הרשימה
> הקודמת מנתה שמונה מותגים בזמן שלמרום יש תנועה על למעלה מעשרים.
> **הכרעת גיל 2026-08-16: קיומו של עמוד מותג אינו תנאי לכתיבה.** מותג עם
> תוכן טוב וחשיפות מותר לכתיבה גם בלי עמוד `/brands/`. מה שקובע הוא
> ההחרגות למטה, ולא מלאי העמודים.

**מותגי ליבה (עמוד מותג + product-category, מעל 10K חשיפות):**
שארפ (Sharp), בלומברג (Blomberg), מג'ימיקס (Magimix), קיטשן אייד (KitchenAid),
האייר (Haier), מולינקס (Moulinex)

**מותגים עם תנועה משמעותית (2K-30K חשיפות):**
דלונגי (De'Longhi), AEG, סמוראי (Samourai), טפאל (Tefal), פיליפס (Philips),
הוּבר (Hoover), זנוסי (Zanussi), קנדי (Candy), באוקנכט (Baucknecht), אינדסיט (Indesit),
פיילוט (Pilot), לוואמט (Lavamat), גרונדיג (Grundig), דה דיטריך (De Dietrich)

**זנב עם תנועה אמיתית (0.5K-1.5K), כולם מוצרי חשמל בשירות מרום:**
אמאנה (Amana), רוזייר (Rosieres), קראון (Crown), פיאבה (Fhiaba), סקולטס (Scholtes),
לה קאסה (La Cassa), קינג (King)

**מוצרי אלקטרוניקה, חלפים בלבד — אין תוכן שירות:**
הייסנס (Hisense), TCL, בוס (Bose), JBL. **מרום אינה נותנת שירות למותגים
האלה.** מה שקיים באתר הוא חלפים בלבד: שלטים לטלוויזיות (הייסנס, TCL)
ורמקולים ואוזניות (בוס, JBL). **אין לכתוב עליהם תוכן שירות, אין לטעון
שירות רשמי, ואין להתייחס אליהם כמותגי מרום.**

> **שים לב ל-TCL:** היא כן מותג שירות של **פלרום** (ביקור טכנאי 340 ₪),
> ולא של מרום. אל תגלוש בין האתרים.

---

### החרגות מחייבות

**בקו (Beko) — אסור לחלוטין.** מרום אינה השירות הרשמי שלה יותר. אין לכתוב
עליה תוכן בשום צורה, למרות 12,131 חשיפות ו-794 קליקים בקטגוריה. נאכף
ב-`BRAND_BEKO`. **זו ההוכחה שחשיפות לבדן אינן מתירות כתיבה.**

**דלונגי — שתי החרגות:**
- **מקררים:** אין קו מקררים דלונגי בישראל. אין להזכיר אותם בתוכן דלונגי.
- **מכונות קפה:** היבואן הוא ברימאג, לא ניופאן. מרום מתקנת אותן בפועל
  אך **אינה השירות הרשמי** להן. אסור לטעון "שירות רשמי" למכונות קפה
  דלונגי. מותר: "מרום מתקנת" + ציון ברימאג כיבואן. מוצרים גדולים
  (תנורים, כיריים, מדיחים, מייבשים, קולטי אדים) מיובאים בידי ניופאן,
  ושם מרום כן השירות הרשמי. נאכף ב-`DELONGHI_COFFEE` וב-`DELONGHI_FRIDGE`.

**מוצרי חשמל קטנים (כל המותגים):** אין ביקור טכנאי בבית ואין שיחת תיאום.
מעבדה בלבד, הלקוח מביא או שולח. חל על מטחנות בשר, מעבדי מזון, בלנדרים,
שואבים, מיקסרים ומגהצי קיטור.

**וייקינג ופיאבה:** מחיר ביקור טכנאי טרם התקבל. אל תנחש מחיר בתוכן שלהם.

### ממצא לתשומת לב (משימת אתר, לא תוכן)

- **`/product-category/hisenes/` קולט שאילתות שירות שאיננו נותנים.**
  העמוד הוא קטגוריית שלטים לטלוויזיות, אבל הוא מדורג על "שירות הייסנס"
  (78 חשיפות, מיקום 12.5), "הייסנס שירות" (26, מיקום 13.2) ו"שירות למקרר
  הייסנס" (20, מיקום 15.1). אלה שאילתות שירות שמרום אינה יכולה לשרת.
  לא לכתוב תוכן שמחזק אותן.
...
מקור אמת יחיד לפרטי המומחה. `author-bio` ו-Person schema נשענים על זה verbatim. Tom Winter (Campixx 2026): "Trust זה לא להוסיף ביוגרפיית כותב" — לכן ה-bio חייב הוכחה, לא תיאור. **הסעיף סגור (הכרעת גיל, 2026-08-16): אלה כל הנתונים שקיימים, ואין מה להמתין לו.** אל תמציא נתונים אישיים ואל תוסיף שנות ניסיון או הסמכות.
...
- **הוכחת ניסיון:** התפקיד "מנהל טכני ויבוא" הוא עוגן הסמכות. אין נתון שנים או היקף, ואין להוסיף אחד.
- **sameAs:** **אין, וזו החלטה ולא פער.** למיכה איתן אין פרסום חיצוני שניתן לאמת. **השמט את שדה `sameAs` כולו** מישות ה-`Person`, אל תשאיר אותו ריק ואל תמלא בקישור פנימי.

> **למה מרום שומרת ישות `Person` ופלרום לא (2026-08-16):** ההבדל אינו ה-`sameAs`,
> שחסר בשתיהן. ההבדל הוא שלמיכה איתן יש **שם אמיתי ותפקיד מאומת** שאפשר לתלות
> בהם `knowsAbout` ועוגן סמכות. בפלרום לא היה שם, ולכן הישות הייתה ריקה.
> ישות אדם עם שם ותפקיד בלי `sameAs` היא חלשה יותר, אבל היא עדיין ישות.
> אם יתקבל בעתיד פרסום חיצוני מאומת, מוסיפים אותו כאן וזה השינוי היחיד שנדרש.
